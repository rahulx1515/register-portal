// src/components/admin/HostelAdminDashboard.jsx
...
export default function HostelAdminDashboard({ user }) {
  ...
  function AmazonLikeProgress({ status }) {
    ...
    return (
      <div className="mt-1 text-[11px] text-gray-500">
        ...
        <div className="flex justify-between text-[10px] font-medium mb-1">
          <span
            className={
              step >= 1 ? "text-indigo-700" : "text-gray-400"
            }
          >
            Received
          </span>
          <span
            className={
              step >= 2 ? "text-indigo-700" : "text-gray-400"
            }
          >
            In Progress
          </span>
          <span
            className={
              step >= 3 ? "text-indigo-700" : "text-gray-400"
            }
          >
            Resolved
          </span>
        ...
        <div className="flex justify-between text-[10px] text-gray-400">
          ...
          <span className="text-right">
            ...
            {step === 2 && "Currently being worked on"}
  ...
  async function handleUpdateStatus(id, newStatus) {
    const raw = prompt(
      "Admin remarks (optional). Click OK to send. Leave empty for 'No remarks provided'.",
      ""
    );
    if (raw === null) {
      return;
  ...
  async function updateStatusWithRemark(id, newStatus, adminRemarks) {
    ...
    try {
      ...
      setComplaints((prev) =>
        prev.map((c) =>
          getId(c) === id
            ? { ...updated, studentName: c.studentName, email: c.email }
            : c
        )
      );
        ...
        <div className="flex items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-semibold text-gray-900">
              Hostel Department Admin
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              Managing complaints for <strong>{collegeCategory}</strong>
            </p>
          </div>
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gradient-to-br from-yellow-50 to-yellow-25 p-4 rounded-2xl shadow-sm border border-yellow-100">
                ...
                <div className="mt-2 text-2xl font-semibold text-yellow-800">
                  {stats.inProgress}
                </div>
                ...
                <div className="space-y-3">
                  {visible.map((c) => {
                    ...
                    return (
                      <article
                        key={id}
                        className="flex flex-col md:flex-row md:items-start justify-between gap-3 p-3 border border-gray-100 rounded-xl"
                      >
                        <div className="md:w-3/4">
                          <div className="flex items-center gap-3 text-xs text-gray-500">
                            ...
                            <div>•</div>
                            <div className="uppercase text-[11px] tracking-wide text-gray-500">
                              {c.category || "Other"}
                            </div>
                          </div>
                          ...
                          {/* Date + badge + Amazon-like progress */}
                          <div className="mt-3 flex flex-col gap-2 text-xs text-gray-400">
                            <div className="flex items-center gap-4">
                              <div>{shortDate(created)}</div>
                              <div className="flex items-center gap-2">
                                <div
                                  className={`inline-flex items-center gap-2 px-2 py-1 rounded-md text-[12px] ${statusToBadge(
                                    c.status || "Open"
                                  )}`}
                                >
                            ...
                            <AmazonLikeProgress status={c.status || "Open"} />
                          </div>
                        ...
                        <div className="md:w-1/4 flex flex-col items-end gap-3">
                          ...
                          <div className="flex flex-col w-full items-end gap-2">
                            ...
                            <button
                              onClick={() => {
                                const full = prompt(
                                  "Full updated message / note (optional):",
                                  ""
                                );
                                if (full !== null) {
                                  alert(
                                    "Note saved (demo). Implement adminNote update server-side if needed."
                                  );
                                }
                              }}
            ...
            <div className="sticky top-6 space-y-4">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
                ...
                <div className="mt-2 text-xs text-gray-500">
                  Export visible complaints to CSV. Use the filters above to
                  refine your export.
                </div>
